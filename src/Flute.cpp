...
Flute :: Flute( StkFloat lowestFrequency )
{
  ...
  filter_.setPole( 0.7f - ( 0.1f * 22050.0f / Stk::sampleRate() ) );
...
void Flute :: setFrequency( StkFloat frequency )
{
  ...
  lastFrequency_ = frequency * 0.66666f;
  ...
  StkFloat delay = Stk::sampleRate() / lastFrequency_ - filter_.phaseDelay( lastFrequency_ ) - 1.0f;
...
void Flute :: startBlowing( StkFloat amplitude, StkFloat rate )
{
  ...
  maxPressure_ = amplitude / (StkFloat) 0.8f;
  adsr_.keyOn();
}
...
void Flute :: controlChange( int number, StkFloat value )
{
  ...
  if (number == __SK_JetDelay_) // 2
    this->setJetDelay( (StkFloat) (0.08f + (0.48f * normalizedValue)) );
